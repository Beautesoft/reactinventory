...
function AddGrn({ docData }) {
    ...
    const initializeData = async () => {
      ...
      try {
        if (urlDocNo) {
          ...
          if (urlStatus != 7) await getStockDetails();
  ...
  }, [itemFilter.skip, debouncedSearchValue]);
  ...
  const getStockDetails = async () => {
    Promise.all([
      apiService.get(`PackageItemDetails${buildFilterQuery(itemFilter)}`),
      apiService.get(`PackageItemDetails/count${buildCountQuery(itemFilter)}`),
    ])

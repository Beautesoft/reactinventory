...
function PurchaseOrder() {
  ...
  const getPOTable = async () => {
    ...
    try {
      ...
      const [dataResponse, countResponse] = await Promise.all([
        apiService.get(`pos${query}`),
        apiService.get(`pos/count${countQuery}`),
      ]);
  ...
  const handleTabChange = (value) => {
    const status = value === "open" ? "Open" : value === "posted" ? "Posted" : value === "approved" ? "Approved" : null;
    setActiveTab(value);
    setPagination((prev) => ({
      ...prev,
      where: {
  ...
  const handleRoute = () => {
    ...
    navigate("/purchase-order/add/"); // Navigate to the add page
  };

  const handleSort = (key, direction) => {
    setPagination((prev) => ({
      ...prev,
      order: `${key} ${direction === 'ascending' ? 'ASC' : 'DESC'}, poNo DESC`,
